...
#
#
# async def auchan(url):
#     async with aiohttp.ClientSession() as session:
#         response = await session.get(url)
#         result = await response.text()
#         soup = BeautifulSoup(result, 'html.parser')
#         price_auchan = soup.find('div',
#                                  class_='styles_price__U1y_f typography_h3__AujKn styles_productCardContentPanel_price__MqlWB')
#         old_price_auchan = soup.find('div',
#                                      class_='styles_price__U1y_f styles_price__oldPrice__VsVTT styles_productCardContentPanel_oldPrice__TYk2W')
#         atributes_auchan = soup.find('a',
#                                      class_='styles_productCardContentPanel_link__6vQup styles_link__VBOJI typography_body__83w8q')
...
#         tasks.append(auchan(base_auchan_url + str(num)))
#         tasks.append(ozon(base_ozon_url + str(num)))
#         tasks.append(wb(base_wb_url + str(num)))
